...
def main():
    ...
    # ret, diff_bf_html = cv2.threshold(diff_bf_html, 0, 255, cv2.THRESH_BINARY + cv2.THRESH_OTSU)

    diff_af_html = cv2.absdiff(img3_gray, img4_gray)
    # ret, diff_af_html = cv2.threshold(diff_af_html, 0, 255, cv2.THRESH_BINARY + cv2.THRESH_OTSU)

    diff_bf_html = cv2.adaptiveThreshold(diff_bf_html, 255, cv2.ADAPTIVE_THRESH_MEAN_C, 
                                    cv2.THRESH_BINARY, 11, 2)
    diff_af_html = cv2.adaptiveThreshold(diff_af_html, 255, cv2.ADAPTIVE_THRESH_MEAN_C, 
                                    cv2.THRESH_BINARY, 11, 2)

    # 白黒を逆にする
    diff_bf_html = cv2.bitwise_not(diff_bf_html)
    diff_af_html = cv2.bitwise_not(diff_af_html)

    """輪郭描画"""
    # diff_bf_html画像がグレースケールの場合、1チャネルにする
    lower_red = np.array([0])  # グレースケールの場合の例
    upper_red = np.array([255])  # グレースケールの場合の例

    # 色範囲に基づいてマスクを作成
    mask_bf = cv2.inRange(diff_bf_html, lower_red, upper_red)
    mask_af = cv2.inRange(diff_af_html, lower_red, upper_red)

    # 赤枠のみを含む画像を生成
    result_bf = np.zeros_like(diff_bf_html)
    result_bf[mask_bf > 0] = diff_bf_html[mask_bf > 0]

    # 緑枠のみを含む画像を生成
    result_af = np.zeros_like(diff_af_html)
    result_af[mask_af > 0] = diff_af_html[mask_af > 0]
