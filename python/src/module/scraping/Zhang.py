...
def zhang_shasha_distance(tree1, tree2):
    def dist(node1, node2):
        if node1.name != node2.name:
            return 1
        return 0

    def zhang_shasha(node1, node2, memo):
        if (node1, node2) in memo:
            return memo[(node1, node2)]
        
        if not node1.children and not node2.children:
            memo[(node1, node2)] = dist(node1, node2)
            return memo[(node1, node2)]

        dp = float('inf')
        if node1.children and node2.children:
            cost = dist(node1, node2)
            dp = min(dp, zhang_shasha(node1.children[0], node2.children[0], memo) + cost)
        
        if node1.children:
            dp = min(dp, zhang_shasha(node1.children[0], node2, memo) + 1)
        
        if node2.children:
            dp = min(dp, zhang_shasha(node1, node2.children[0], memo) + 1)
        
        memo[(node1, node2)] = dp
        return dp

    memo = {}
    return zhang_shasha(tree1, tree2, memo)
...
# Zhang-Shashaアルゴリズムを使ってDOMツリーを比較
before_apt_tree = build_tree(before_tree)
after_apt_tree = build_tree(after_tree)

edit_distance = zhang_shasha_distance(before_apt_tree, after_apt_tree)

print(f"Zhang-Shasha Distance: {edit_distance}")
