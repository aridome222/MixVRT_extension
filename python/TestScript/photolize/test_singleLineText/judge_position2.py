...
for contour in all_contours:
    ...
    for text_position in text_positions:
        ...
        if dist < threshold_distance:  # 適切な距離の閾値を設定
            ...
            text_position[2] = max(text_position[2], x)  # 枠の左上座標を更新
            text_position[3] = max(text_position[3], y)
            text_position[4] = min(text_position[4], x + w)  # 枠の右下座標を更新
            text_position[5] = min(text_position[5], y + h)
...
for position in text_positions:
    x, y, x1, y1, x2, y2 = position[2:] ### ここでエラーが起こる、文字が少し離れていると一つの文字列としてみなされず枠が１つ１つ囲まれてしまう
    cv2.rectangle(img2, (x, y), (x2, y2), (0, 255, 0), 2)  # 緑色の枠を描画
