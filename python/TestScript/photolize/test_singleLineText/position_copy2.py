...
### 変更前と変更後の色分け ###
# 白色の範囲を定義
lower_white = np.array([128, 128, 128])  # 下限（B、G、R）
upper_white = np.array([255, 255, 255])  # 上限（B、G、R）

# 白色の範囲内にあるピクセルをマスクとして取得
white_mask = cv2.inRange(result_bin1_rgb, lower_white, upper_white)
white_mask = cv2.inRange(result_bin2_rgb, lower_white, upper_white)
...
# ピクセルの色を変更
result_bin1_rgb[white_mask > 0] = green_color
result_bin2_rgb[white_mask > 0] = red_color
...
# 画像を保存する
cv2.imwrite(output_file_path, result_add2)
