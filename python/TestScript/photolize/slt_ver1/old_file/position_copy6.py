...
edges2 = cv2.Canny(result_bin2, threshold1, threshold2)

# 二値画像をRGB形式に変換
result_bin1_rgb = cv2.cvtColor(edges1, cv2.COLOR_GRAY2RGB)
result_bin2_rgb = cv2.cvtColor(edges2, cv2.COLOR_GRAY2RGB)
...
# 輪郭の近似
epsilon = 0.02 * cv2.arcLength(contours1[0], True)  # イプシロンの値を調整して近似の滑らかさを調整
approx1 = cv2.approxPolyDP(contours1[0], epsilon, True)

epsilon = 0.02 * cv2.arcLength(contours2[0], True)
approx2 = cv2.approxPolyDP(contours2[0], epsilon, True)

# 滑らかな輪郭を含む新しい画像を作成
result_contour1 = np.zeros_like(result_bin1_rgb)
result_contour2 = np.zeros_like(result_bin2_rgb)

# 輪郭を描画
cv2.drawContours(result_contour1, [approx1], -1, (0, 0, 255), thickness=2)
cv2.drawContours(result_contour2, [approx2], -1, (0, 0, 255), thickness=2)

# 二値画像をRGB形式に変換し、滑らかな輪郭を含む画像と重ねる
result_smooth1 = cv2.add(result_bin1_rgb, result_contour1)
result_smooth2 = cv2.add(result_bin2_rgb, result_contour2)
...
white_mask1 = cv2.inRange(result_smooth1, lower_white, upper_white)
white_mask2 = cv2.inRange(result_smooth2, lower_white, upper_white)
...
# ピクセルの色を変更
result_smooth1[white_mask1 > 0] = red_color
result_smooth2[white_mask2 > 0] = green_color

# 二値画像をRGB形式に変換し、2枚の画像を重ねる。
result_diff = cv2.add(result_smooth1, result_smooth2)
result = cv2.addWeighted(img1, 0.2, result_diff, 0.8, 2.2) # ２.２はガンマ値。大きくすると白っぽくなる
